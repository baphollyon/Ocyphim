...
let optionSorter = (options) => {
    ...
    let scrubbedSheetId = sheetID ? sheetID.includes('/d/') ? sheetID.split('/d/')[1].split('/edit')[0] : sheetID : "10KjN_xzqbYB9cB4kncmo61hKL673s_-iXfkEXOY_BAg";

    // Call all options, make defaults of our own
    let userOptions = options;
    let defaultOptions = {

        sheetID: 1P43fbVE88aWGqFtIdJpiX7uvHOTnAedamHWBdfgTrzc,
...
let sheetPage = (id, pageName) => {
    return `https://docs.google.com/spreadsheets/d/${id}/gviz/tq?tqx=out:json&headers=1&tq=WHERE A IS NOT NULL&sheet=${Ocydex}`
};
